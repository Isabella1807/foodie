...
En prisliste over, hvad hver ting flytter på måldatoen, delt i to: ting der sker ÉN gang (en sprunget træning, en dag over målet, en sprunget hyggedag) og ting der sker HVER dag fremover (100 kcal mere om dagen, syv timer i stedet for seks). Forskellen er pointen: én sprunget træning er 14 timer, men 100 kcal mere hver dag er to måneder. Overskriften på den nederste liste siger måldatoen med ("Hver uge, hele vejen til juli 2028"), og teksten nedenunder gentager, at den kun gælder ved en vane — ellers læser man "+2,4 uger" som prisen for én uge. **Begge lister har to kolonner: i kalorier og i tid.** Kalorierne er det egentlige; tiden er kun den ene måde at betale dem på. Den anden er mindre mad, fordi appen sætter dagsmålet ned, når den måler et lavere forbrug. Det er derfor tids-tallet ser lille ud, og uden kalorie-kolonnen ligner "kun fem timer om ugen = +2 uger" en gratis frokost. Engangs-ting regnes som kalorier delt med dagens underskud; varige ting regnes ved at køre hele kurven igen med et andet dagligt regnskab. Under en dag skrives i timer, over to uger i uger, over to måneder i måneder. Se `planPrices` i `src/stores/data.js`.

## "Træning i planen" — træningens længde og antal dage
Før stod der fast "en time, seks dage om ugen" i koden. Laver man i virkeligheden 45 minutter, passede måldatoen ikke, og hver enkelt 45-minutters gang faldt under dagsgrænsen og talte som nul. Nu sættes begge dele under "Mine mål" (`plan_minutes`, `plan_days`), og alt følger med: hvad én træning er værd, grænsen for at en dag tæller, ugens mål, hyggedagens manglende træning, prislisten og kurven. Med hendes tal: 60 min × 6 dage = juli 2028, 45 min × 6 = august 2028, 45 min × 7 = juli 2028 (altså det samme som en time seks dage om ugen). Kræver kolonnerne — se bunden af `sql/schema.sql`.
...
## "Appen må ikke gå under" — bunden under det tal, appen sætter
Det automatiske dagsmål er forbruget minus underskuddet. Falder forbruget, falder målet — også når det falder, fordi der blev sprunget en træning over. Uden en bund ville mindre motion altså betyde mindre mad, og det er en dårlig handel for den, der i forvejen ikke spiser meget. **Beskyttelsen er slået til som standard**: bunden er dit eget daglige mål (`kcal_goal`), med mindre man selv sætter `min_kcal`. Appen må altså gerne give MERE at spise, når forbrændingen tillader det, men aldrig mindre end det, man selv har valgt. Før var standarden 1200, så appen kunne skære i maden uden at nogen havde bedt om det — det var forkert, og en bruger skal ikke selv finde og tænde et værn mod noget, appen ikke burde gøre. Rammer regnestykket bunden, og går planen helt i stå, SIGER kortet det ("går i stå omkring 66,6 kg") i stedet for at stå tomt. **Det er en bund under APPENS tal, ikke en regel for hvad brugeren skal spise** — feltet hed først "Spis aldrig under", og det blev læst som en ordre om at tvinge mad ned på dage uden appetit. Rammer målet bunden, rykker MÅLDATOEN i stedet, og maden bliver stående. Appen går aldrig under 1200 uanset hvad. Bunden bruges både af dagsmålet og af plan-kurven, så måldatoen viser den rigtige pris. Eksempel med hendes tal: bund 1200 giver juli 2028, bund 1400 giver januar 2029, bund 1500 giver september 2030. Kræver kolonnen `min_kcal` — se bunden af `sql/schema.sql`.

### Hvis jeg spiser (på Plan-fanen, starter foldet sammen)
